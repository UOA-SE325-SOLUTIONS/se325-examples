...
//    private static final Logger LOGGER = LoggerFactory.getLogger(ParoleeWebServiceIT.class);
    ...
    public void updateConvictions() {
        ...
        Set<Conviction> convictionsForOliver = CLIENT.target(targetUri).request()
        ...
        Set<Conviction> reQueriedConvictions = CLIENT.target(targetUri).request()
                .accept(MediaType.APPLICATION_JSON).get(new GenericType<>() {
                });
        ...
        for (Conviction c : convictionsForOliver) {
            assertTrue(reQueriedConvictions.contains(c));
        }
    }
